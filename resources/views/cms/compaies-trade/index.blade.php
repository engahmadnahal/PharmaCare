...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header border-0 py-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">{{__('cms.trademark_company')}}</span>
            <span class="text-muted mt-3 font-weight-bold font-size-sm"></span>
        </h3>
        @can('Create-Country')
        <div class="card-toolbar">
            <a href="{{route('tradeCompaies.create')}}"
            ...
            <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_2">
                <thead>
                    <tr class="text-uppercase">
                        ...
                        <th style="min-width: 150px">{{__('cms.trademark')}}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="pl-0">
                            <a href="#"
                                class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$company->translations->first()?->name
                                ?? ''}}</a>
                        </td>
                        <td>
                            <span class="label label-lg label-light-info label-inline">{{$company->trades_count}}</span>
                        </td>
                        <td>
                            <a href="#" data-toggle="modal" data-target="#company_{{$company->id}}_translations"
                                class="btn btn-light-primary font-weight-bolder font-size-sm">({{$company->translations_count}})</a>
                            <div class="modal fade" id="company_{{$company->id}}_translations" tabindex="-1"
                                role="dialog" aria-labelledby="company_{{$company->id}}_translations"
                                aria-hidden="true">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="pr-0 text-right">
                                                            @can('Update-Country')
                                                            <a href="{{route('companyTrade-translations.edit',$translation->id)}}"
                        ...
                        <td>
                            <span
                                class="label label-lg @if($company->active) label-light-success @else label-light-warning @endif label-inline">{{$company->active_key}}</span>
                        </td>
                        <td class="pr-0 text-right">
                            @can('Create-Country')
                            <a href="{{route('companyTrade-translations.create',$company->id)}}"
                                class="btn btn-icon btn-light btn-hover-primary btn-sm">
                            ...
                            @endcan
                            @can('Delete-Country')
                            <a href="#" onclick="performCountryDestroy('{{$company->id}}', this)"
                                class="btn btn-icon btn-light btn-hover-primary btn-sm">
                                <span class="svg-icon svg-icon-md svg-icon-primary">
                                    <!--begin::Svg Icon | path:assets/media/svg/icons/General/Trash.svg-->
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
...
<script>
    function performCountryDestroy(id,reference) {
        confirmDestroy('/cms/admin/tradeCompaies', id, reference);
    }
    function performTranslationDestroy(id,reference) {
        confirmDestroy('/cms/admin/tradeCompaies/translations', id, reference);
    }
</script>
@endsection