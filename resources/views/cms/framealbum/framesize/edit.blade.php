            ...
            <div class="card card-custom gutter-b example example-compact">
                ...
                <form id="create-form">
                    <div class="card-body">
                                ...
                                <div class="dropdown bootstrap-select form-control dropup">
                                    <select class="form-control selectpicker" data-size="7" id="frames_or_album_id"
                                        ...
                                        @foreach ($frameOrAlbum as $s)
                                            <option value="{{ $s->id }}" @selected($s->id == $data->frames_or_album_id)>
                                                {{ $s->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <x-input type="number" name="{{ __('cms.width') }}" id="width" value="{{ $data->width }}" />
                        <x-input type="number" name="{{ __('cms.height') }}" id="height" value="{{ $data->height }}" />
                        {{-- @foreach ($currency as $crr)
                            <x-input type="text" name="{{ __('cms.price') }} {{ $crr->name }}"
                                id="price_{{ $crr->id }}"
                                value="{{ $data->price->where('currency_id', $crr->id)->first()?->price }}" />
                        @endforeach --}}
...
@section('scripts')
    <script>
        ...
        function performEdit(id) {
            ...
            store('/cms/admin/frames_sizes/' + id, formData, '/cms/admin/frames_sizes');
        }
    </script>
@endsection
