            ...
            <form id="create-form">
                <div class="card-body">
                            ...
                            <div class="dropdown bootstrap-select form-control dropup">
                                <select class="form-control selectpicker" data-size="7" id="language"
                                    ...
                                    @foreach ($languages as $language)
                                        <option value="{{$language->id}}" @selected($job->language_id == $language->id)>{{$language->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="form-group row mt-4">
                        ...
                        <div class="col-9">
                            <input type="text" class="form-control" id="name" placeholder="{{__('cms.name')}}" value="{{$job->name}}"/>
...
<script>
    function performEdit(){
        ...
        update('/cms/admin/jobs/translations/{{$job->id}}', data, '/cms/admin/jobs');
    }
</script>
@endsection