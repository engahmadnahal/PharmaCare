            ...
            <form id="create-form">
                <div class="card-body">
                    <div class="form-group row mt-4">
                        <label class="col-3 col-form-label">{{__('cms.poster')}}:<span
                                class="text-danger">*</span></label>
                        <div class="col-lg-4 col-md-9 col-sm-12">
                            <div class="dropdown bootstrap-select form-control dropup">
                                <select class="form-control selectpicker" data-size="7" id="posterprint_service_id"
                                    title="Choose one of the following..." tabindex="null" data-live-search="true">
                                    @foreach ($poster as $s)
                                    <option value="{{$s->id}}" @selected($s->id == $data->posterprint_service_id)>{{$s->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="form-group row mt-4">
                        <label class="col-3 col-form-label">{{__('cms.height')}}:</label>
                        <div class="col-9">
                            <input type="number" class="form-control" id="height" placeholder="{{__('cms.height')}}" value="{{$data->height}}"/>
                            <span class="form-text text-muted">{{__('cms.please_enter')}} {{__('cms.height')}} </span>
                        </div>
                    </div>

                    <div class="form-group row mt-4">
                        <label class="col-3 col-form-label">{{__('cms.width')}}:</label>
                        <div class="col-9">
                            <input type="number" class="form-control" id="width" placeholder="{{__('cms.width')}}" value="{{$data->width}}"/>
                            <span class="form-text text-muted">{{__('cms.please_enter')}} {{__('cms.width')}} </span>
                        </div>
                    </div>
...
<script>
    ...
    function performEdit(id){
        ...
        formData.append('posterprint_service_id',document.getElementById('posterprint_service_id').value);
        ...
        store('/cms/admin/option_posterprint_services/'+id, formData, '/cms/admin/option_posterprint_services');
    }
</script>
@endsection