        ...
        <div class="card card-custom gutter-b example example-compact">
            ...
            <form id="create-form">
                <div class="card-body">
                    ...
                    <x-input id="price" type="text" name="{{ __('cms.price') }}" value="{{$storeDelivaryBalance->price}}"/>
                    <x-input id="duration_num" type="text" name="{{ __('cms.duration_num') }}" value="{{$storeDelivaryBalance->duration_num}}"/>
                        ...
                        <div class="col-lg-4 col-md-9 col-sm-12">
                            <div class="dropdown bootstrap-select form-control dropup">
                                <select class="form-control selectpicker" data-size="7" id="duration_type"
                                    title="Choose one of the following..." tabindex="null" data-live-search="true">
                                        <option value="houre" @selected($storeDelivaryBalance->duration_type == 'houre')>{{__('cms.houre')}}</option>
                                        <option value="day" @selected($storeDelivaryBalance->duration_type == 'day')>{{__('cms.day')}}</option>
                                        <option value="week" @selected($storeDelivaryBalance->duration_type == 'week')>{{__('cms.week')}}</option>
                                        <option value="month" @selected($storeDelivaryBalance->duration_type == 'month')>{{__('cms.month')}}</option>
                                </select>
                            </div>
                            <span class="form-text text-muted">{{__('cms.please_select')}}
...
<script>
    function performUpdate(){
        ...
        update('/cms/admin/store_delivary_balances/{{$storeDelivaryBalance->id}}', data, '/cms/admin/store_delivary_balances');
    }
</script>
@endsection