...
@section('page-name', __('cms.financial_online'))
@section('main-page', __('cms.reports'))
@section('sub-page', __('cms.financial_online'))
    ...
    <div class="card card-custom gutter-b">
        ...
        <div class="card-header border-0 py-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label font-weight-bolder text-dark">{{ __('cms.financial_online') }}</span>
            ...
            <div class="card-toolbar">
                ...
                <div class="actions" style="width: 100%;display: flex;justify-content: end;">
                    ...
                    <a href="{{ route('reports.financial.cash.excel') }}"
                    ...
                    <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_2">
                        <thead>
                            <tr class="text-uppercase">
                                ...
                                <th style="max-width: 120px">{{ __('cms.photome_rate') }} (25%)</th>
                                <th style="max-width: 120px">{{ __('cms.banck_fees') }} (2.5%)</th>
                                <th style="max-width: 120px">{{ __('cms.net_value') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            @foreach ($data as $std)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="text-dark-75 font-weight-bolder d-block font-size-lg">{{ $std->photome_rate }}
                                            {{ $std->currency?->code }}</span>
                                    </td>

                                    <td>
                                        <span
                                            class="text-dark-75 font-weight-bolder d-block font-size-lg">{{ $std->fees }}
                                            {{ $std->currency?->code }}</span>
                                    </td>
    
                                    <td>
                                        <span
                                            class="text-dark-75 font-weight-bolder d-block font-size-lg">{{ $std->net_value }}
                                            {{ $std->currency?->code }}</span>
                                    </td>
        ...
        <script>
            function performFilter() {
                let from = $('#from').val(),
                    ...
                    `{{ route('reports.financial.online') }}?from=${from}&to=${to}`;
            }
        </script>
    @endsection
